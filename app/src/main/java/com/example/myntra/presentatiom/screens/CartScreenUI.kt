...
@Composable
fun CartScreenUI(viewModel: ViewModel = hiltViewModel(),navController: NavHostController,productviewmodel: productviewmodel) {
    val state = viewModel.getcartProducts.value
...
Text("Cart Products")
                LazyColumn(
                    ...
                    if (!products.isNullOrEmpty()) {
                        items(products.size){
                            ...
                            AllProductCartSCreenCard(product = products[it], navController,productviewmodel)
                        }
                ...
                Row(modifier = Modifier.fillMaxWidth()) {
                    Text(text = "Total Price", fontSize = 20.sp, color = Color.Black)
                    Spacer(modifier = Modifier.width(100.dp))
                    Text(text = "Rs${totalprice.value}", fontSize = 20.sp, color = Color.Black)
                }
                if(!products.isNullOrEmpty()) {
                    Button(
                        onClick = {},
...
@Composable
fun AllProductCartSCreenCard(product: ProductModel, navController: NavHostController, productviemodel: productviewmodel,viewModel: ViewModel= hiltViewModel()) {
    ...
    Card(modifier = Modifier.fillMaxWidth().height(110.dp).clickable {
        ...
        Row(modifier = Modifier.fillMaxWidth()){
            ...
            Spacer(modifier = Modifier.width(30.dp))
                ...
                Icon(painter = painterResource(R.drawable.baseline_delete_24), modifier = Modifier.clickable { viewModel.removefromcart(product.id)
                                                                                                             navController.navigate(Routes.CartScreen)}, contentDescription = null, tint = Color.Red)
